...
const submitReply = async () => {
  try {
    ...
    // API로 댓글 등록 요청
    const response = await axios.post('http://erpc-final-backend-env.eba-i73jvuqm.ap-northeast-2.elasticbeanstalk.com/notice_comment/regist', {
...
const removeReply = async (commentId) => {
  try {
    if (confirm('댓글을 삭제하시겠습니까?')) {
      await axios.patch(`http://erpc-final-backend-env.eba-i73jvuqm.ap-northeast-2.elasticbeanstalk.com/notice_comment/delete/${commentId}`);
      // 삭제된 댓글을 화면에서 제거
...
onMounted(async () => {
  try {
    const noticeId = router.currentRoute.value.params.noticeId;
    // 해당 공지에 대한 댓글 가져오기
    const response = await axios.get(`http://erpc-final-backend-env.eba-i73jvuqm.ap-northeast-2.elasticbeanstalk.com/notice_comment`);
    // 가져온 댓글을 화면에 표시
