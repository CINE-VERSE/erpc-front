...
export default {
  ...
  methods: {
    ...
    async modifyNotice() {
      ...
      try {
        const response = await axios.patch(
          `http://erpc-final-backend-env.eba-i73jvuqm.ap-northeast-2.elasticbeanstalk.com/notice_board/modify/${this.noticeId}`,
          formData,
          {
            headers: {
    ...
    async fetchNoticeDetails() {
      try {
        const response = await axios.get(`http://erpc-final-backend-env.eba-i73jvuqm.ap-northeast-2.elasticbeanstalk.com/notice_board/${this.noticeId}`);
